...
class HMM:
    ...
    def forward(self, sequence):
        ...
        # and with transitions["#"].keys() rows to account for all states, including "#"
        matrix = [[float(0) for j in range(len(sequence) + 1)] for i in range(len(self.transitions.keys()))]
        matrix[0][0] = 1.0
            ...
            if i == 1 :
                 ...
                 for idx1, state in enumerate(self.transitions.keys()) :
                    ...
                    for idx2, state2 in enumerate(self.transitions.keys()) :
                        ...
                        try:
                            prob += matrix[idx2][i-1] * float(self.transitions[state2][state]) * float(self.emissions[state][sequence[i - 1]])
                        # if KeyError, means transition or emission is not reachable therefore is 0
                        except KeyError:
                            prob += 0
                    matrix[idx1][i] = prob
        ...
        for idx, row in enumerate(matrix):
            if row[len(row) - 1] > max_val:
                ...
                max_idx = idx
        return list(self.transitions.keys())[max_idx]

    def viterbi(self, sequence):
        # Initializing a matrix with sequence + 1 columns to account for sequence and #
        # and with transitions["#"].keys() + 1 rows to account for starting states and "#"
        matrix = [[float(0) for j in range(len(sequence) + 1)] for i in range(len(self.transitions.keys()))]
        backpointers = [[float(0) for j in range(len(sequence) + 1)] for i in range(len(self.transitions.keys()))]
        matrix[0][0] = 1.0
        # Iterating through columns -> sequence
        for i in range(1, len(sequence) + 1):
            if i == 1:
            ...
            # Iterating through rows (states) -> "#", "happy", "grumpy", "hungry"
                for idx1, state in enumerate(self.transitions.keys()):
                    ...
                    for idx2, state2 in enumerate(self.transitions.keys()):
                        try :
                            curr_prob = matrix[idx2][i - 1] * float(self.transitions[state2][state]) * float(self.emissions[state][sequence[i - 1]])
                        # if KeyError, means transition or emission is not reachable therefore is 0
                        except KeyError:
                            curr_prob = 0
                        if curr_prob > max_prob:
                            max_prob = curr_prob
                            backpointers[idx1][i] = idx2
                    matrix[idx1][i] = max_prob
        states = []
        min_value = float("inf")
        min_row = -1
        ...
        for state in states[1:] :
            emits.append(list(self.transitions.keys())[state])
        return emits

def safe_land(state) :
    return state in ["4,3", "4,4", "3,4", "2,5"]

def run(basename, file, type) :
    h = HMM()
    h.load(basename)
        ...
        for line in f :
            if len(line) != 1 :
                ...
                match type :
                    ...
                    case "viterbi" :
                        likely_states = h.viterbi(tokens)
                        print(f"The most likely sequence of hidden states for the sequence of observations \"%s\" is \"%s\"" % (" ".join(tokens), " ".join(likely_states)))
                        if basename == "lander" :
                            if safe_land(likely_states[-1]) :
                                print("It it safe for the lander to land")
                            else :
                                print("It is not safe for the lander to land")
...
if __name__ == "__main__" :
    ...
    if args.forward :
        run(args.basename, args.forward, "forward")
    if args.viterbi :
        run(args.basename, args.viterbi, "viterbi")
    if args.generate :
        h = HMM()
        with open(args.basename + "_sequence.obs", 'w') as f :
            h.load(args.basename)
            tokens = " ".join(h.generate(int(args.generate)))
            f.write(tokens)
